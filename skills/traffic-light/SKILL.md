---
name: traffic-light
description: Start, stop, inspect, or configure the Codex Traffic Light Windows status overlay. Use when the user asks to launch the traffic light, close it, check its state, or enable/disable startup.
---
...
The overlay is read-only. Never change or delete Codex session files.

## State Meaning

- Yellow steady: one or more Codex turns are active.
- Yellow flashing: a command approval or user response is pending.
- Green: all observed turns have completed.
- Red: no readable Codex session was found or the monitor encountered an error.
