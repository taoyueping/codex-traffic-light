...
if ($Probe) {
    Get-CodexTrafficState | ConvertTo-Json -Compress
    exit 0
}
...
$createdNew = $false
$mutex = [System.Threading.Mutex]::new($true, "Local\CodexTrafficLight", [ref]$createdNew)
if (-not $createdNew) {
    exit 0
}
...
function Set-LampState {
    ...
    $window.ToolTip = "Codex Traffic Light`n$Label`nDrag to move; right-click to exit"
}
